...
def postprocess(frame, outs, classes, yolo):
    ...
    def drawPred(classId, conf, left, top, right, bottom):
        # Increase the bounding box thickness here
        thickness = 5
        # Draw a bounding box.
        ...
        labelSize, baseLine = cv.getTextSize(label, cv.FONT_HERSHEY_SIMPLEX, 2, 1)
        top = max(top, labelSize[1])
        cv.rectangle(frame, (left, top - labelSize[1]), (left + labelSize[0], top + baseLine), (255, 255, 255), cv.FILLED)
        cv.putText(frame, label, (left, top), cv.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0))
    ...
    for i in indices:
        ...
        width = box[2]
        height = box[3]
         # Get classId for current detection
        classId = classIds[i]

        # Update the dictionary with count of each detected class
        if classes[classId] in label_counts:
            label_counts[classes[classId]] += 1
        else:
            label_counts[classes[classId]] = 1

        drawPred(classIds[i], confidences[i], left, top, left + width, top + height)
    print("Detected labels and counts:", label_counts)